...
### 출력 
 ...
 처음에는 bufferedwriter 에서 개행을 newLine()으로 하다가 오류가 남. 
 
 	// bw의 write(int)는 int 값의 유니코드 값을 저장한다. 
				// 식 안에 String 값에 해당하는 "\n"을 넣어줬을 때는 
				// write(String)가 실행되어 보기에 정상적인 값이 출력되었지만
				// write(int)형태에서는 해당 유니코드의 값을 가진 문자가 출력된다.
    // 결과적으로 개행문자의 형태 때문에 ""의 삽입위치가 달라졌고, 그로 인해 결과가 달라졌으니
    그 원인을 \n과 newline()의 차이로 보았던 것이다.
				
				
				// write(int) 해당 메소드의 목적을 int를 입력하기 위한 것이라고 생각했는데, 
				//그게 아니라 수로 된 값으로 문자를 입력받기 위한 메소드였다. (착각) 그래서 이해가 안갔던 것.
